...
typedef struct{
    qxwz_u16_t  auth_port;
    qxwz_u16_t  rtcm_port;
    qxwz_s8_t*  auth_serv;
    qxwz_s8_t*  rtcm_serv;
}qxwz_server_cfg_t;
...
/**
 * @brief Set the configuration of user private network
 * @param[in]  server_cfg: the configuration of user private network
 *
 * @return:
 *   0 if success
 *  -1 if fail
 */
qxwz_s32_t qxwz_set_servercfg(qxwz_server_cfg_t* server_cfg);

/**
 * @brief Set the configuration of gga upload freqency
 * @param[in]  upload_freq: gga upload freqency
 *
 * @return:
 *   0 if success
 *  -1 if fail
 */
qxwz_s32_t qxwz_set_gga_upload_freq(qxwz_u32_t upload_freq);
